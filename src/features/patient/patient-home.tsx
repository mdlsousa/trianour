  ...
  return (
    <div className="relative px-4 pt-6">
      ...
      >
        ...
        <p className="mt-1 text-4xl font-semibold">{weeklyAdherence.score}%</p>
        <Progress
          value={weeklyAdherence.score}
          className="mt-4 h-2 bg-white/20 [&>div]:bg-white"
        />
        <div className="mt-4 flex gap-4 text-sm opacity-90">
          <span className="flex items-center gap-1">
            <Flame className="h-4 w-4" /> {weeklyAdherence.mealsCompleted}/{weeklyAdherence.mealsTotal} refeições
          </span>
          <span className="flex items-center gap-1">
          ...
          <Card className="overflow-hidden border-primary/20">
            <div className={`bg-gradient-to-br ${nextMeal.image} p-1`}>
              <CardContent className="rounded-[calc(1rem-4px)] bg-card/95 p-5 backdrop-blur">
                ...
                <p className="text-sm text-muted-foreground">{nextMeal.time} · {nextMeal.calories} kcal</p>
                {nextMeal.trainingContext && (
                  <p className="mt-2 text-xs text-primary">{nextMeal.trainingContext}</p>
                )}
      ...
      <div className="mt-6">
        ...
        <div className="mt-3 space-y-2">
          {todayMeals.map((meal) => (
            <div
              key={meal.id}
              className="flex items-center gap-3 rounded-2xl border border-border/50 bg-card/80 p-4 backdrop-blur"
            >
      ...
      <Card className="mt-6">
        <CardContent className="p-5">
          ...
          <Progress value={hydration.current / hydration.goal * 100} className="mt-3" />
